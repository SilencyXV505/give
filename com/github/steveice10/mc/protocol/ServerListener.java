                ...
                switch (packet.getIntent()) {
                    ...
                    case LOGIN:
                        ...
                        if (packet.getProtocolVersion() > MinecraftConstants.PROTOCOL_VERSION) {
                            event.getSession().disconnect(Component.text("Outdated server! I'm still on " + MinecraftConstants.GAME_VERSION + "."));
                        } else if (packet.getProtocolVersion() < MinecraftConstants.PROTOCOL_VERSION) {
                            event.getSession().disconnect(Component.text("Outdated client! Please use " + MinecraftConstants.GAME_VERSION + "."));
                        }
        ...
        if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
            if (event.getPacket() instanceof LoginStartPacket) {
                ...
                if (!Arrays.equals(this.verifyToken, packet.getVerifyToken(privateKey))) {
                    event.getSession().disconnect(Component.text("Invalid nonce!"));
                    return;
                }
        ...
        public void run() {
            ...
            if (this.key != null) {
                ...
                try {
                    ...
                    this.session.disconnect(Component.text("Failed to make session service request."), e);
                    return;
                }

                if (profile == null) {
                    this.session.disconnect(Component.text("Failed to verify username."));
                }
            } else {
