        ...
        if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
            if (event.getPacket() instanceof EncryptionRequestPacket) {
                ...
                try {
                    ...
                    event.getSession().disconnect(Component.text("Login failed: Authentication service unavailable."), e);
                    return;
                } catch (InvalidCredentialsException e) {
                    event.getSession().disconnect(Component.text("Login failed: Invalid login session."), e);
                    return;
                } catch (RequestException e) {
                    event.getSession().disconnect(Component.text("Login failed: Authentication error: " + e.getMessage()), e);
                    return;
                }
                ...
                event.getSession().disconnect(event.<LoginDisconnectPacket>getPacket().getReason());
            ...
            if (event.getPacket() instanceof StatusResponsePacket) {
                ...
                event.getSession().disconnect(Component.text("Finished"));
            }
